...
#define FPL_GFX_THROW_INFO_ONLY(call) infoManager.Set(); (call); {auto v = infoManager.GetMessages(); if(!v.empty()) {throw Graphics::InfoException( __LINE__,__FILE__,v);}}
#else
#define FPL_GFX_EXCEPT( hr ) Graphics::HrException(__LINE__,__FILE__,(hr))
#define FPL_GFX_THROW_INFO(hrcall) FPL_GFX_THROW_NOINFO(hrcall)
#define FPL_GFX_DEVICE_REMOVED_EXCEPT(hr) Graphics::DeviceRemovedException(__LINE__,__FILE__,(hr)) 
#define FPL_GFX_THROW_INFO_ONLY(call) (call)
#endif
